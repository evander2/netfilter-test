	...
	if (ret >= 0)
	{
		...
		for(int i = 0; i < http_end; i++){
			if(memcmp(data+i, stp, 8) == 0){
				...
				host.check = memcmp(data+host_start, host.real_host, host_len);
				host.check = host.check?0:1;
				break;
			}
		}
	}
	      ...
	      struct nfq_data *nfa, void *data)
{
	...
	return nfq_set_verdict(qh, id, host.check?NF_DROP:NF_ACCEPT, 0, NULL);
}
